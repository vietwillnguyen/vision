...
9. **Data dir** — `mkdir -p` the directory the daemon will actually record into, which is `VISIO_DATA_DIR` from the env file when set and `/var/lib/visio-recorder` otherwise. Provisioning the configured one rather than the default matters because preflight checks the same value: creating only the default would leave preflight blocking on a directory nothing ever creates, and advising a re-run that would not create it either.
...
| Data dir writable | the daemon's own data dir - `VISIO_DATA_DIR` if set, else `/var/lib/visio-recorder`, read with the same `.get` precedence as `daemon.load_config` - exists and is writable | block, except **exists-but-unwritable for a non-daemon caller**, which is warn - see below |
