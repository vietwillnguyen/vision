...
2. **apt packages** — required: `rpicam-apps`, `zbar-tools`, `ffmpeg`, `network-manager`, `git`. Best-effort: `pijuice-base`, which replaces the hardware plan's stray `pip install pijuice` (it installs into system Python, which matters once the daemon runs as root against system-level hardware libraries rather than a project venv for those specific libraries — see `drivers.py`'s deferred-import convention). `pijuice-base` is installed only when it is present in the running release's package index: the Raspberry Pi archive ships it for bookworm but dropped it in trixie, and per the non-negotiable above a missing battery driver must never fail setup. The script probes `apt-cache policy` rather than ignoring the install's exit code, so a genuine install failure still aborts.
...
7. **Developer shell** — editor and shell conveniences for bring-up over SSH: `neovim` (also set as the `editor` alternative and as `EDITOR`/`VISUAL`), `ripgrep`, `fd-find`, `tmux`, `htop`, `tree`, `jq`, `less`, `bash-completion`, plus a managed block in the *invoking* user's `~/.bashrc` (resolved from `SUDO_USER`, not root's `$HOME`) carrying the standard `ls` aliases, colour, history settings, and a git-branch-aware prompt. Gated on `VISIO_DEV_SHELL` in the env file, defaulting to enabled; set it to `0` for a shipped appliance image. The `~/.bashrc` block is delimited and rewritten in place, so re-running is byte-identical rather than appending. Packages are probed against the release's index with the same pattern as `pijuice-base`.
8. **Env file** — if `/etc/visio-recorder.env` doesn't exist, write a template with `SUPABASE_URL=`/`SUPABASE_ANON_KEY=` placeholders plus `VISIO_BATTERY_SOURCE=none` and `VISIO_DEV_SHELL=1` (both commented, explaining the bring-up defaults) and exit with an actionable message. If it exists, leave it untouched (never overwrite real secrets) and just verify the required keys are present and non-placeholder.
