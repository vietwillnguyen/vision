...
8. **Developer shell** — editor and shell conveniences for bring-up over SSH: `neovim` (also set as the `editor` alternative and as `EDITOR`/`VISUAL`), `ripgrep`, `fd-find`, `tmux`, `htop`, `tree`, `jq`, `less`, `bash-completion`, plus a managed block in the *invoking* user's `~/.bashrc` (resolved from `SUDO_USER`, not root's `$HOME`) carrying the standard `ls` aliases, colour, history settings, and a git-branch-aware prompt. Gated on `VISIO_DEV_SHELL` in the env file, defaulting to enabled; set it to `0` for a shipped appliance image. The switch covers these editor and interactive-shell conveniences and nothing else - notably not step 5's `build-essential`/`python3-dev`, which a device installs whenever it has to compile `rpi_ws281x`, since no archive packages that library today. The `~/.bashrc` block is delimited and rewritten in place, so re-running is byte-identical rather than appending, and disabling the switch removes the block rather than merely stopping future updates - packages already installed are deliberately left alone, so pre-seeding the env file is the only path to a device that never carries these conveniences. Packages are probed against the release's index with the same pattern as `pijuice-base`. Resolving the invoking user's home and group is guarded: an unresolvable account skips this optional step rather than aborting provisioning before the data dir and summary.
9. **Data dir** — `mkdir -p` the directory the daemon will actually record into, which is `VISIO_DATA_DIR` from the env file when set and `/var/lib/visio-recorder` otherwise. Provisioning the configured one rather than the default matters because preflight checks the same value: creating only the default would leave preflight blocking on a directory nothing ever creates, and advising a re-run that would not create it either. The value is operator input rather than a constant this script owns, so it is validated - a non-absolute path is rejected outright, since it would resolve against the script's cwd here but against the unit's `WorkingDirectory=` for the daemon - and a failed `mkdir` is reported and carried to the summary rather than aborting, because step 10 is where the operator finds out what happened.
10. **Summary** — report what changed, including the exact provisioned commit SHA, the data dir, and the `visio-preflight` command; if step 3 changed anything, explicitly say a reboot is required before the daemon will work. Also echoes the configured `VISIO_BATTERY_SOURCE` and, when it's `none`, a one-line reminder that this is a bring-up-only configuration - never a hard failure, since a missing PiJuice HAT is an accepted, current-known state, not a setup error. A step-5 `rpi_ws281x` build that failed is repeated here as an explicit `INCOMPLETE` line, carried in a flag the way the reboot notice is: the mid-run warning is thousands of lines of apt output away by the time an operator reads the summary. The last line and the exit status then follow those flags - `Setup incomplete` and exit 1 when any of them is set, `Setup complete.` and exit 0 otherwise - so neither the operator's last line nor a caller testing `$?` reports success for a device that cannot record or cannot start. This is distinct from the env-file step's exit 0, which is a deliberate re-run checkpoint rather than a failure: "not finished yet, by design" and "finished but broken" are different answers and get different statuses.
...
| Data dir writable | the daemon's own data dir - `VISIO_DATA_DIR` from the environment, else from `/etc/visio-recorder.env`, else `/var/lib/visio-recorder`; `daemon.load_config` calls the same resolver, so there is no second default to keep in step - exists and is writable | block, except **exists-but-unwritable for a non-daemon caller**, which is warn - see below |
...
A failed import names the step that actually installs that module rather than a single generic "check the uv sync step": only `supabase` comes from `uv.lock`, `gpiozero` is apt's `python3-gpiozero` reached through the venv's `--system-site-packages`, and `rpi_ws281x` is step 5's source build.

Which directory gets checked is caller-relative too, and for the same reason. `ExecStartPre` inherits `VISIO_DATA_DIR` from the unit's `EnvironmentFile=`; the standalone `visio-preflight` inherits nothing, so it reads `/etc/visio-recorder.env` itself. That file is mode 600 root-owned and stays that way - it is not relaxed for a diagnostic's benefit - so an unprivileged run cannot see a configured value. It checks the default and says so on the `data_dir` line rather than reporting a guess as a verified fact, which would show a blocking failure for a directory that is correctly absent on a device recording to a USB mount. Severity is untouched by the caveat: which directory was checked, and how certain that choice is, are separate facts from whether the directory that was checked is usable.
