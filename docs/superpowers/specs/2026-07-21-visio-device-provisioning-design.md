...
1. **apt packages** — required: `rpicam-apps`, `zbar-tools`, `ffmpeg`, `network-manager`, `git`. Best-effort: `pijuice-base`, which replaces the hardware plan's stray `pip install pijuice` (it installs into system Python, which matters once the daemon runs as root against system-level hardware libraries rather than a project venv for those specific libraries — see `drivers.py`'s deferred-import convention). `pijuice-base` is installed only when it is present in the running release's package index: the Raspberry Pi archive ships it for bookworm but dropped it in trixie, and per the non-negotiable above a missing battery driver must never fail setup. The script probes `apt-cache policy` rather than ignoring the install's exit code, so a genuine install failure still aborts.
