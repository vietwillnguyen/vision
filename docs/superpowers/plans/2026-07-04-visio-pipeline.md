...
**As-built note (issue #5):** the orchestrator landed as `pipeline/pipeline/orchestrator.py` (pure, unit-tested against `Protocol` boundaries) with `pipeline/pipeline/__main__.py` as the single place real adapters (`pipeline/pipeline/adapters/`) are wired, mirroring the firmware's `daemon.py` convention.
It is deployed as the `nightly-reel` GitHub Actions workflow (nightly cron at 19:00 UTC plus `workflow_dispatch` with an optional `day` input), not a Supabase Edge Function or Lambda - Edge Functions run Deno and the pipeline is Python + FFmpeg; the pending-segment-count threshold trigger was not built.
The DLQ policy below also landed: rejected and unmatched keys persist to the `pipeline_dlq` table (migration `20260716090000`) with attempt counts, are retried every nightly run, and at 5 attempts the key is escalated and the device owner is push-notified.
