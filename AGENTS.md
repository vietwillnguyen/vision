...
PRs are gated by `.github/workflows/tests.yml` (pytest, jest, web-bundle smoke, pgTAP), `.github/workflows/lint.yml` (ruff check, ruff format, mypy, eslint), and `.github/workflows/hooks.yml` (the pre-commit hook's own suite plus a re-verification of the `docs/ARCHITECTURE.html` manifest); `tsc --noEmit` runs in both tests.yml and lint.yml, so a type error reddens both.
